-- Ensure admins can query all jobs and optimize status lookups
BEGIN;

-- Index to speed up status/approval filters
CREATE INDEX IF NOT EXISTS jobs_active_approval_idx
  ON public.jobs (is_active, is_approved, is_rejected);

-- Policy allowing admins / super admins to read every job row.
-- Drop first to avoid duplicate-policy errors when re-running migrations.
DROP POLICY IF EXISTS "Admins can view all jobs" ON public.jobs;
CREATE POLICY "Admins can view all jobs" ON public.jobs
  FOR SELECT
  USING (
    (coalesce(current_setting('request.jwt.claims', true)::jsonb->>'role', '')) IN ('admin', 'super_admin')
  );

-- Admin-focused RPC for querying jobs across all statuses with pagination
