-- 20251214_update_group_invite_rate_limits.sql
-- Adjust invite_member_by_email rate limits: 20 invites per inviter/group/day, 3 per target email/day
...
declare
  ...
  v_email_hash    text := md5(lower(coalesce(p_email, '')));
begin
  ...
  -- Rate limits
  perform public.increment_group_rate_limit('group_invite_global', 20, '1 day');
  perform public.increment_group_rate_limit('group_invite_group_' || p_group_id::text, 20, '1 day');
  perform public.increment_group_rate_limit('group_invite_email_' || v_email_hash, 3, '1 day');
