...
CREATE OR REPLACE FUNCTION "public"."app_is_admin"() RETURNS boolean
    ...
    SET "search_path" TO 'public'
    AS $$
  select public.claim_role() in ('admin','super_admin');
$$;
...
CREATE OR REPLACE FUNCTION "public"."app_is_admin_of"("p_user" "uuid" DEFAULT "auth"."uid"()) RETURNS boolean
...
CREATE OR REPLACE FUNCTION "public"."connections_notify"() RETURNS "trigger"
    ...
    AS $$
begin
  -- assume connections has: id, requester_id, target_id, status
  if new.status = 'pending' then
    perform public.notify_connection_request(new.target_id, new.requester_id, new.id);
  end if;
  return null;
end$$;
...
CREATE OR REPLACE FUNCTION "public"."create_group_and_add_admin"("group_name" "text", "group_description" "text" DEFAULT ''::"text", "group_is_private" boolean DEFAULT false, "group_tags" "text"[] DEFAULT '{}'::"text"[]) RETURNS "uuid"
    ...
    AS $$
DECLARE
  v_norm text := lower(regexp_replace(btrim(group_name), '\s+',' ', 'g'));
  v_group_id uuid;
BEGIN
  -- Only block ACTIVE duplicates
  IF EXISTS (
    SELECT 1 FROM public.groups
    WHERE name_norm = v_norm AND is_archived = false
  ) THEN
    RAISE EXCEPTION USING
      ERRCODE = 'unique_violation',
      MESSAGE = 'Name already in use. Please choose a different name.',
      DETAIL  = 'name_conflict';
  END IF;

  INSERT INTO public.groups (name, description, is_private, is_archived, is_approved)
  VALUES (group_name, COALESCE(group_description, ''), COALESCE(group_is_private, false), false, true)
  RETURNING id INTO v_group_id;
...
CREATE OR REPLACE FUNCTION "public"."current_role_text"() RETURNS "text"
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
  select public.claim_role();
$$;


ALTER FUNCTION "public"."current_role_text"() OWNER TO "postgres";
...
begin
  ...
  where least(user_a, user_b) = least(u1, u2)
    and greatest(user_a, user_b) = greatest(u1, u2)
  limit 1;
...
CREATE OR REPLACE FUNCTION "public"."dm_messages_notify"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
begin
  -- assume dm_messages has: thread_id, sender_id, recipient_id, content
  perform public.notify_chat_message(new.recipient_id, new.thread_id, left(coalesce(new.content,''), 120));
  return null;
end$$;


ALTER FUNCTION "public"."dm_messages_notify"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."get_or_create_dm_thread"("user1" "uuid", "user2" "uuid") RETURNS "uuid"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
...
begin
  if user1 = user2 then
    raise exception 'Cannot create a thread with yourself';
  end if;
...
ALTER FUNCTION "public"."get_or_create_dm_thread"("user1" "uuid", "user2" "uuid") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."handle_new_user"() RETURNS "trigger"
    ...
    AS $$
begin
  insert into public.profiles (id, email, show_in_directory, created_at)
...
CREATE OR REPLACE FUNCTION "public"."is_employer"("p_user_id" "uuid") RETURNS boolean
    ...
    SET "search_path" TO 'public'
    AS $$
  select public.current_role()::text in ('employer','admin','super_admin');
$$;
...
CREATE OR REPLACE FUNCTION "public"."notify_on_dm_message"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
begin
  -- notify the recipient with a deep link to open this thread
  insert into public.notifications (
    recipient_id, sender_id, type, title, message, link, metadata
  ) values (
    NEW.recipient_id,
    NEW.sender_id,
    'message',
    ...
    '/messages?tab=chats&open=' || NEW.thread_id,
...
ALTER FUNCTION "public"."notify_on_dm_message"() OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."group_memberships" (
    ...
    "status" "public"."membership_status_enum" DEFAULT 'approved'::"public"."membership_status_enum" NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL
);
...
CREATE OR REPLACE VIEW "public"."v_mentors_public" AS
 SELECT "m"."id",
   ...
   FROM ("public"."mentors" "m"
     JOIN "public"."profiles" "p" ON (("p"."id" = "m"."user_id")))
  WHERE (("m"."status" = 'approved'::"text") AND ("p"."approval_status" = 'approved'::"public"."profile_approval_status") AND (COALESCE("p"."is_available_for_mentorship", false) = true));
...
CREATE OR REPLACE VIEW "public"."v_my_dm_threads" AS
 SELECT "t"."id" AS "thread_id",
    ...
    COALESCE("p"."full_name", TRIM(BOTH FROM ((COALESCE("p"."first_name", ''::"text") || ' '::"text") || COALESCE("p"."last_name", ''::"text"))), 'User'::"text") AS "other_user_name",
...
CREATE OR REPLACE TRIGGER "trg_dm_messages_notify" AFTER INSERT ON "public"."dm_messages" FOR EACH ROW EXECUTE FUNCTION "public"."dm_messages_notify"();
...
CREATE OR REPLACE TRIGGER "trg_notify_on_dm_message" AFTER INSERT ON "public"."dm_messages" FOR EACH ROW EXECUTE FUNCTION "public"."notify_on_dm_message"();
...
CREATE POLICY "anon_select_companies_basic" ON "public"."companies" FOR SELECT TO "anon" USING (true);
...
CREATE POLICY "auth_insert_companies" ON "public"."companies" FOR INSERT TO "authenticated" WITH CHECK (("created_by" = "auth"."uid"()));
...
CREATE POLICY "companies.insert.admin" ON "public"."companies" FOR INSERT TO "authenticated" WITH CHECK ("public"."is_user_admin"("auth"."uid"()));



CREATE POLICY "companies.insert.employer_or_admin" ON "public"."companies" FOR INSERT TO "authenticated" WITH CHECK ((("public"."current_role"() = ANY (ARRAY['employer'::"public"."app_role_enum", 'admin'::"public"."app_role_enum", 'super_admin'::"public"."app_role_enum"])) AND ("created_by" = "auth"."uid"())));
...
CREATE POLICY "companies.update.owner_or_admin" ON "public"."companies" FOR UPDATE TO "authenticated" USING ((("created_by" = "auth"."uid"()) OR ("public"."current_role"() = ANY (ARRAY['admin'::"public"."app_role_enum", 'super_admin'::"public"."app_role_enum"])))) WITH CHECK ((("created_by" = "auth"."uid"()) OR ("public"."current_role"() = ANY (ARRAY['admin'::"public"."app_role_enum", 'super_admin'::"public"."app_role_enum"]))));



CREATE POLICY "companies_select_via_jobs_applied" ON "public"."companies" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."jobs" "j"
     JOIN "public"."job_applications" "ja" ON (("ja"."job_id" = "j"."id")))
  WHERE (("j"."company_id" = "companies"."id") AND ("ja"."applicant_id" = "auth"."uid"())))));



CREATE POLICY "companies_update_own_logo" ON "public"."companies" FOR UPDATE TO "authenticated" USING (("created_by" = "auth"."uid"())) WITH CHECK (("created_by" = "auth"."uid"()));
...
CREATE POLICY "dm_insert_when_connected" ON "public"."dm_messages" FOR INSERT WITH CHECK ((("sender_id" = "auth"."uid"()) AND (EXISTS ( SELECT 1
   FROM (("public"."conversation_participants" "me"
     JOIN "public"."conversation_participants" "other" ON ((("other"."conversation_id" = "me"."conversation_id") AND ("other"."user_id" <> "me"."user_id"))))
     JOIN "public"."connections" "c" ON ((((("c"."requester_id" = "me"."user_id") AND ("c"."recipient_id" = "other"."user_id")) OR (("c"."requester_id" = "other"."user_id") AND ("c"."recipient_id" = "me"."user_id"))) AND ("c"."status" = 'accepted'::"text"))))
  WHERE (("me"."conversation_id" = "dm_messages"."thread_id") AND ("me"."user_id" = "auth"."uid"()))))));
...
CREATE POLICY "dm_messages_insert_participant" ON "public"."dm_messages" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."dm_threads" "t"
  WHERE (("t"."id" = "dm_messages"."thread_id") AND (("t"."user_a" = "auth"."uid"()) OR ("t"."user_b" = "auth"."uid"()))))));



CREATE POLICY "dm_messages_insert_when_connected" ON "public"."dm_messages" FOR INSERT WITH CHECK ((("sender_id" = "auth"."uid"()) AND (EXISTS ( SELECT 1
   FROM "public"."dm_threads" "t"
  WHERE (("t"."id" = "dm_messages"."thread_id") AND (("t"."user_a" = "auth"."uid"()) OR ("t"."user_b" = "auth"."uid"())) AND "public"."are_connected"("auth"."uid"(),
        CASE
            WHEN ("t"."user_a" = "auth"."uid"()) THEN "t"."user_b"
            ELSE "t"."user_a"
        END))))));
...
CREATE POLICY "dm_messages_select_participant" ON "public"."dm_messages" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."dm_threads" "t"
  WHERE (("t"."id" = "dm_messages"."thread_id") AND (("t"."user_a" = "auth"."uid"()) OR ("t"."user_b" = "auth"."uid"()))))));



CREATE POLICY "dm_select_participant" ON "public"."dm_messages" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."conversation_participants" "p"
  WHERE (("p"."conversation_id" = "dm_messages"."thread_id") AND ("p"."user_id" = "auth"."uid"())))));
...
CREATE POLICY "dm_threads_select" ON "public"."dm_threads" FOR SELECT TO "authenticated" USING (("public"."is_user_admin"("auth"."uid"()) OR (("auth"."uid"() = "user_a") OR ("auth"."uid"() = "user_b"))));



CREATE POLICY "dm_threads_select_mine" ON "public"."dm_threads" FOR SELECT USING ((("user_a" = "auth"."uid"()) OR ("user_b" = "auth"."uid"())));



CREATE POLICY "dm_threads_select_participant" ON "public"."dm_threads" FOR SELECT TO "authenticated" USING ((("user_a" = "auth"."uid"()) OR ("user_b" = "auth"."uid"())));
...
CREATE POLICY "gm_delete" ON "public"."group_members" FOR DELETE TO "authenticated" USING ((("user_id" = "auth"."uid"()) OR "public"."is_user_admin"("auth"."uid"()) OR "public"."is_group_admin"("auth"."uid"(), "group_id")));
...
CREATE POLICY "gm_insert" ON "public"."group_members" FOR INSERT TO "authenticated" WITH CHECK (((("user_id" = "auth"."uid"()) AND (EXISTS ( SELECT 1
   FROM "public"."groups" "g"
  WHERE (("g"."id" = "group_members"."group_id") AND ("g"."is_private" = false) AND ("g"."is_approved" = true) AND ("g"."is_archived" = false))))) OR "public"."is_user_admin"("auth"."uid"()) OR "public"."is_group_admin"("auth"."uid"(), "group_id")));



CREATE POLICY "gm_insert" ON "public"."group_memberships" FOR INSERT TO "authenticated" WITH CHECK ((("user_id" = "auth"."uid"()) AND ((EXISTS ( SELECT 1
   FROM "public"."groups" "g"
  WHERE (("g"."id" = "group_memberships"."group_id") AND ("g"."visibility" = 'public'::"public"."group_visibility_enum") AND ("g"."is_approved" = true)))) OR (EXISTS ( SELECT 1
   FROM "public"."groups" "g"
  WHERE (("g"."id" = "group_memberships"."group_id") AND ("g"."visibility" = 'private'::"public"."group_visibility_enum")))))));
...
CREATE POLICY "gm_insert_creator" ON "public"."group_memberships" FOR INSERT TO "authenticated" WITH CHECK ((("user_id" = "auth"."uid"()) AND (EXISTS ( SELECT 1
   FROM "public"."groups" "g"
  WHERE (("g"."id" = "group_memberships"."group_id") AND ("g"."created_by" = "auth"."uid"()))))));



CREATE POLICY "gm_select" ON "public"."group_members" FOR SELECT TO "authenticated" USING ((("user_id" = "auth"."uid"()) OR "public"."is_user_admin"("auth"."uid"()) OR "public"."is_group_admin"("auth"."uid"(), "group_id")));



CREATE POLICY "gm_select" ON "public"."group_memberships" FOR SELECT TO "authenticated" USING (("public"."is_user_admin"("auth"."uid"()) OR ("user_id" = "auth"."uid"()) OR "public"."is_group_member"("group_id")));



CREATE POLICY "gm_update" ON "public"."group_memberships" FOR UPDATE TO "authenticated" USING (("public"."is_user_admin"("auth"."uid"()) OR "public"."is_group_manager"("group_id"))) WITH CHECK (true);



CREATE POLICY "gm_update_roles" ON "public"."group_members" FOR UPDATE TO "authenticated" USING (("public"."is_user_admin"("auth"."uid"()) OR "public"."is_group_admin"("auth"."uid"(), "group_id"))) WITH CHECK (("public"."is_user_admin"("auth"."uid"()) OR "public"."is_group_admin"("auth"."uid"(), "group_id")));



CREATE POLICY "gp_delete" ON "public"."group_posts" FOR DELETE TO "authenticated" USING ((("user_id" = "auth"."uid"()) OR "public"."is_user_admin"("auth"."uid"()) OR "public"."is_group_admin"("auth"."uid"(), "group_id")));
...
CREATE POLICY "gp_insert" ON "public"."group_posts" FOR INSERT TO "authenticated" WITH CHECK ((("user_id" = "auth"."uid"()) AND (EXISTS ( SELECT 1
  ...
  WHERE (("gm"."group_id" = "group_posts"."group_id") AND ("gm"."user_id" = "auth"."uid"())))) AND (EXISTS ( SELECT 1
   FROM "public"."groups" "g"
  WHERE (("g"."id" = "group_posts"."group_id") AND ("g"."is_archived" = false) AND ((NOT "g"."is_admin_only_posts") OR "public"."is_user_admin"("auth"."uid"()) OR (EXISTS ( SELECT 1
           FROM "public"."group_members" "gm2"
          WHERE (("gm2"."group_id" = "group_posts"."group_id") AND ("gm2"."user_id" = "auth"."uid"()) AND ("gm2"."role" = 'admin'::"text"))))))))));



CREATE POLICY "gp_select" ON "public"."group_posts" FOR SELECT TO "authenticated" USING (("public"."is_user_admin"("auth"."uid"()) OR ((EXISTS ( SELECT 1
   FROM "public"."groups" "g"
...
CREATE POLICY "gp_update" ON "public"."group_posts" FOR UPDATE TO "authenticated" USING ((("user_id" = "auth"."uid"()) OR "public"."is_user_admin"("auth"."uid"()) OR "public"."is_group_admin"("auth"."uid"(), "group_id"))) WITH CHECK ((("user_id" = "auth"."uid"()) OR "public"."is_user_admin"("auth"."uid"()) OR "public"."is_group_admin"("auth"."uid"(), "group_id")));
...
CREATE POLICY "group_members_join" ON "public"."group_members" FOR INSERT TO "authenticated" WITH CHECK (("user_id" = "auth"."uid"()));
...
CREATE POLICY "groups_select" ON "public"."groups" FOR SELECT TO "authenticated" USING (("public"."is_user_admin"("auth"."uid"()) OR (("visibility" = 'public'::"public"."group_visibility_enum") AND ("is_approved" = true)) OR ("created_by" = "auth"."uid"()) OR "public"."is_group_member"("id")));
...
CREATE POLICY "groups_update_admin" ON "public"."groups" FOR UPDATE TO "authenticated" USING ((("created_by" = "auth"."uid"()) OR "public"."is_user_admin"("auth"."uid"()) OR (EXISTS ( SELECT 1
   FROM "public"."group_members" "gm"
  WHERE (("gm"."group_id" = "groups"."id") AND ("gm"."user_id" = "auth"."uid"()) AND ("gm"."role" = ANY (ARRAY['owner'::"text", 'admin'::"text"]))))))) WITH CHECK (true);
...
CREATE POLICY "jobs_select" ON "public"."jobs" FOR SELECT TO "authenticated" USING (((COALESCE("is_active", false) AND COALESCE("is_approved", true)) OR ("posted_by" = "auth"."uid"()) OR ("created_by" = "auth"."uid"()) OR "public"."fc_is_admin"() OR "public"."fc_is_employer"()));



CREATE POLICY "jobs_select_admin_all" ON "public"."jobs" FOR SELECT TO "authenticated" USING ((COALESCE((("current_setting"('request.jwt.claims'::"text", true))::"jsonb" ->> 'role'::"text"), 'alumni'::"text") = ANY (ARRAY['admin'::"text", 'super_admin'::"text"])));
...
CREATE POLICY "jobs_select_public_active" ON "public"."jobs" FOR SELECT TO "authenticated" USING ((("is_active" = true) AND ("is_approved" = true) AND (("deadline" IS NULL) OR ("deadline" >= "now"()))));
...
CREATE POLICY "owner_update_companies" ON "public"."companies" FOR UPDATE TO "authenticated" USING (("created_by" = "auth"."uid"())) WITH CHECK (("created_by" = "auth"."uid"()));
...
ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."event_attendees";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."event_rsvps";
...
ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."job_bookmarks";
...
ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."mentorship_messages";
...
GRANT ALL ON FUNCTION "public"."current_role_text"() TO "anon";
GRANT ALL ON FUNCTION "public"."current_role_text"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."current_role_text"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."dm_messages_notify"() TO "anon";
GRANT ALL ON FUNCTION "public"."dm_messages_notify"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."dm_messages_notify"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_or_create_dm_thread"("user1" "uuid", "user2" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_or_create_dm_thread"("user1" "uuid", "user2" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_or_create_dm_thread"("user1" "uuid", "user2" "uuid") TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."is_employer"("p_user_id" "uuid") FROM PUBLIC;
...
GRANT ALL ON FUNCTION "public"."notify_on_dm_message"() TO "anon";
GRANT ALL ON FUNCTION "public"."notify_on_dm_message"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."notify_on_dm_message"() TO "service_role";
