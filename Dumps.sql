...
begin
  update public.profiles
     set approval_status = new_status::public.profile_approval_status,
         approved_at    = case when new_status::public.profile_approval_status = 'approved'
...
begin
  select (p.role in ('admin','super_admin')) into is_admin
  from public.profiles p
  where p.id = auth.uid();

  if coalesce(is_admin, false) then
    return new; -- admins can change everything
  end if;

  -- for non-admins, block approval & review column changes
  if new.is_approved    is distinct from old.is_approved
     or new.approval_status is distinct from old.approval_status
     or new.reviewed_by  is distinct from old.reviewed_by
     or new.reviewed_at  is distinct from old.reviewed_at
  then
    raise exception 'Only admins can change approval/review fields';
  end if;
...
CREATE OR REPLACE VIEW "public"."directory_profiles_base" WITH ("security_barrier"='true') AS
 SELECT "p"."id",
    "p"."first_name",
    "p"."last_name",
    "p"."graduation_year",
   ...
   FROM "public"."profiles" "p"
  WHERE ((COALESCE("p"."is_deleted", false) = false) AND (COALESCE("p"."show_in_directory", true) = true));
...
CREATE OR REPLACE FUNCTION "public"."get_directory_profiles"() RETURNS SETOF "public"."directory_profiles_base"
    ...
    AS $$
DECLARE
  v_role text := public.get_user_role();        -- uses auth.uid()
  v_is_admin boolean := public.is_admin();      -- uses auth.uid()
BEGIN
  -- Employers: block completely
  IF v_role = 'employer' THEN
    RETURN;
  END IF;

  IF v_is_admin THEN
    -- Admins: see everyone (including employers, flagged by is_employer)
    RETURN QUERY
    SELECT * FROM public.directory_profiles_base;
  ELSE
    -- Non-admins: hide employers
    RETURN QUERY
    SELECT * FROM public.directory_profiles_base
...
CREATE OR REPLACE FUNCTION "public"."get_jobs_with_bookmarks_v3"("p_search" "text" DEFAULT NULL::"text", "p_limit" integer DEFAULT 20, "p_offset" integer DEFAULT 0) RETURNS "jsonb"
    LANGUAGE "sql" STABLE SECURITY DEFINER
    AS $$
  WITH base AS (
    SELECT *
    FROM public.v_jobs_public_left
  ...
  with_flags AS (
    SELECT
      b.*,
      /* if you have job_bookmarks, LEFT JOIN it here;
         else just use FALSE: */
  ...
  SELECT jsonb_build_object(
    'items',       COALESCE(jsonb_agg(to_jsonb(paged) ORDER BY created_at DESC)
...
ALTER FUNCTION "public"."get_jobs_with_bookmarks_v3"("p_search" "text", "p_limit" integer, "p_offset" integer) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_jobs_with_bookmarks_v3"("p_search_query" "text" DEFAULT NULL::"text", "p_sort_by" "text" DEFAULT 'created_at'::"text", "p_sort_order" "text" DEFAULT 'desc'::"text", "p_limit" integer DEFAULT 20, "p_offset" integer DEFAULT 0) RETURNS TABLE("id" "uuid", "title" "text", "company_name" "text", "company_logo_url" "text", "location" "text", "job_type" "text", "experience_level" "text", "salary_min" bigint, "salary_max" bigint, "application_deadline" timestamp with time zone, "is_active" boolean, "is_approved" boolean, "created_at" timestamp with time zone, "application_url" "text", "source_type" "text", "applicant_count" bigint, "is_bookmarked" boolean, "total_count" bigint)
    LANGUAGE "sql" STABLE
    AS $$
  with base as (
    select
...
ALTER FUNCTION "public"."get_jobs_with_bookmarks_v3"("p_search_query" "text", "p_sort_by" "text", "p_sort_order" "text", "p_limit" integer, "p_offset" integer) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_jobs_with_bookmarks_v3"("p_search_query" "text" DEFAULT NULL::"text", "p_sort_by" "text" DEFAULT 'created_at'::"text", "p_sort_order" "text" DEFAULT 'desc'::"text", "p_limit" integer DEFAULT 12, "p_offset" integer DEFAULT 0, "p_department" "text" DEFAULT NULL::"text") RETURNS "json"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $_$
DECLARE
  v_sort_col text;
...
BEGIN
  ...
  q := format($f$
    WITH base AS (
      SELECT j.*, c.name AS company_name, c.logo_url AS company_logo_url,
        COALESCE(a.count,0) AS applicant_count,
        EXISTS(SELECT 1 FROM job_bookmarks jb WHERE jb.job_id = j.id AND jb.user_id = auth.uid()) AS is_bookmarked
      FROM public.jobs j
      LEFT JOIN public.companies c ON c.id = j.company_id
      LEFT JOIN (
      ...
      WHERE (
        ...
        AND (%L = '' OR j.title ILIKE '%%' || %L || '%%' OR j.description ILIKE '%%' || %L || '%%'
             OR j.location ILIKE '%%' || %L || '%%' OR c.name ILIKE '%%' || %L || '%%')
  ...
  $f$,
    COALESCE(p_search_query, ''), COALESCE(p_search_query, ''), COALESCE(p_search_query, ''),
    COALESCE(p_search_query, ''), COALESCE(p_search_query, ''),  -- total 5 occurrences
...
ALTER FUNCTION "public"."get_jobs_with_bookmarks_v3"("p_search_query" "text", "p_sort_by" "text", "p_sort_order" "text", "p_limit" integer, "p_offset" integer, "p_department" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."get_my_posted_jobs"("p_limit" integer DEFAULT 20, "p_offset" integer DEFAULT 0) RETURNS TABLE("id" "uuid", "title" "text", "company_id" "uuid", "company_name" "text", "company_logo_url" "text", "is_approved" boolean, "is_active" boolean, "created_at" timestamp with time zone, "updated_at" timestamp with time zone, "total_count" bigint)
    LANGUAGE "sql" STABLE SECURITY DEFINER
    AS $$
WITH mine AS (
  SELECT
...
ALTER FUNCTION "public"."get_my_posted_jobs"("p_limit" integer, "p_offset" integer) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."get_recent_activity"() RETURNS TABLE("activity" "text", "created_at" timestamp with time zone)
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
  -- return an empty set; your dashboard will render a blank widget instead of erroring
  select null::text as activity, now() - interval '1 day' as created_at
  where false;
$$;


ALTER FUNCTION "public"."get_recent_activity"() OWNER TO "postgres";
...
begin
  ...
  /*
    Exactly the activities you requested:
    1) RSVP to an event (accepted/rejected)
    2) Applied for a job
    3) Sent/updated a connection (pending/accepted/rejected/disconnected)
    4) Joined/left a group
    5) Mentorship (mentor sees mentee-related updates; mentee sees mentor-related updates)
  */

  return query
  with
  -- 1) Event RSVPs for this user (limit to explicit accepted/rejected)
  rsvp_feed as (
    select
      ...
      jsonb_build_object('attendance_status', lower(ea.attendance_status)) as meta
    from event_attendees ea
    join events e on e.id = ea.event_id
    where ea.user_id = v_user
      and lower(ea.attendance_status) in ('going','not_going','accepted','rejected')
  ),
  ...
  jobs_feed as (
    select
      ...
      '/jobs/'||j.id::text,
      ja.created_at,
      jsonb_build_object('application_status', coalesce(ja.status,'submitted')) as meta
    from job_applications ja
    join jobs j on j.id = ja.job_id
    where ja.applicant_id = v_user
  ),

  -- 3) Connections touching this user (request sent/accepted/rejected/disconnected)
  connections_feed as (
    select
      ('connection_'||
        case
          when c.status in ('pending') then 'sent'
          when c.status in ('accepted','connected') then 'accepted'
          when c.status = 'rejected' then 'rejected'
          when c.status in ('disconnected','removed') then 'disconnected'
          else 'updated'
        end)::text as activity_type,
      'Connection update'::text              as title,
      c.id                                   as ref_id,
      '/directory'                           as url,
      greatest(c.updated_at, c.created_at)   as created_at,
      jsonb_build_object(
        'peer_id', case when c.requester_id = v_user then c.recipient_id else c.requester_id end,
        'status', c.status
      ) as meta
    from connections c
    where c.requester_id = v_user or c.recipient_id = v_user
  ),

  -- 4) Group joins/leaves
  -- Joins are present in group_members. Leaves typically require an activity log or trigger.
  -- We include joins directly; "leave" entries are read from activity_log if present.
  groups_join_feed as (
    select
      'group_joined'::text,
      g.name,
      ...
      '/groups/'||g.id::text,
      gm.created_at,
      jsonb_build_object('role', gm.role) as meta
    from group_members gm
    join groups g on g.id = gm.group_id
    where gm.user_id = v_user
  ),

  groups_leave_feed as (
    -- Optional: if you log leaves into activity_log (recommended), surface them.
    select
      'group_left'::text,
      coalesce((metadata->>'group_name')::text, 'Left a group') as title,
      coalesce((metadata->>'group_id')::uuid, null) as ref_id,
      case when (metadata ? 'group_id') then '/groups/'||(metadata->>'group_id') else '/groups' end as url,
      created_at,
      metadata as meta
    from activity_log
    where activity_type = 'group_left'
      and user_id = v_user
  ),

  -- 5) Mentorship updates (mentor’s view vs mentee’s view)
  mentorship_feed as (
    -- Mentor perspective: requests involving me as mentor
    select
      'mentor_update'::text,
      coalesce('Mentee update','Mentee update') as title,
      mr.id as ref_id,
      '/mentorship/requests/'||mr.id::text as url,
      mr.created_at,
      jsonb_build_object('status', mr.status, 'mentee_id', mr.mentee_id, 'mentor_id', mr.mentor_id) as meta
    from mentorship_requests mr
    where v_role in ('admin','super_admin','alumni','mentor','faculty') -- widen if you have explicit 'mentor' role
      and mr.mentor_id = v_user

    union all

    -- Mentee perspective: my requests with mentors
    select
      'mentee_update'::text,
      coalesce('Mentor update','Mentor update') as title,
      mr.id as ref_id,
      '/mentorship/requests/'||mr.id::text as url,
      mr.created_at,
      jsonb_build_object('status', mr.status, 'mentee_id', mr.mentee_id, 'mentor_id', mr.mentor_id) as meta
    from mentorship_requests mr
    where mr.mentee_id = v_user
  )

  select * from (
    select * from rsvp_feed
    union all select * from jobs_feed
    union all select * from connections_feed
    union all select * from groups_join_feed
    union all select * from groups_leave_feed
    union all select * from mentorship_feed
  ) all_items
  order by created_at desc
  limit p_limit;
...
CREATE OR REPLACE FUNCTION "public"."get_unread_notifications_count_by_type"() RETURNS TABLE("type" "text", "unread_count" integer)
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
  select coalesce(n.type,'general') as type, count(*)::int as unread_count
  from public.notifications n
  where n.user_id = auth.uid() and n.read_at is null
  group by coalesce(n.type,'general');
$$;
...
declare
  ...
  v_first  text;
  v_last   text;
  v_role   app_role_enum := 'alumni';  -- default, must exist in enum
  v_meta   text := nullif(new.raw_user_meta_data->>'role','');
begin
  -- parse full name (Google/LinkedIn often supply 'name' or 'full_name')
  if v_full <> '' then
    v_first := split_part(v_full, ' ', 1);
    v_last  := nullif(btrim(substring(v_full from length(v_first)+1)), '');
  end if;

  -- set role only if metadata value is a valid enum label
  if v_meta is not null and v_meta in (
      select enumlabel from pg_enum where enumtypid = 'app_role_enum'::regtype
  ) then
    v_role := v_meta::app_role_enum;
  end if;

  insert into public.profiles (
    id, email, role, first_name, last_name, location,
    avatar_url, approval_status, created_at
  )
  values (
    new.id,
...
CREATE OR REPLACE FUNCTION "public"."jobs_set_owner"() RETURNS "trigger"
    ...
    AS $$
begin
  -- Force ownership to current user and lock down approval on insert
  if new.posted_by is null then
    new.posted_by := auth.uid();
  else
    -- Even if client sent something, overwrite to current user to be safe
    new.posted_by := auth.uid();
  end if;

  -- Clients cannot set is_approved on insert; force default
  new.is_approved := coalesce(new.is_approved, false);

  -- Keep is_active default unless explicitly set by owner/admin later
...
BEGIN
  IF NEW.status = 'accepted' THEN
    ...
    VALUES (
      NEW.requester_id,  -- profile_id (legacy)
      NEW.requester_id,  -- recipient_id (canonical)
      'Connection Accepted',
...
CREATE OR REPLACE FUNCTION "public"."notify_event_rsvp"() RETURNS "trigger"
    ...
    AS $$
begin
  insert into notifications (profile_id, recipient_id, title, message, link, type)
  values (
    new.user_id,  -- profile_id (legacy)
    new.user_id,  -- recipient (canonical)
    'RSVP Confirmed',
    'You are registered for the event.',
    '/events/' || new.event_id,
    'event'      -- <— set to the same canonical type
  );
  return new;
end;
$$;
...
CREATE OR REPLACE FUNCTION "public"."request_connection"("p_job_id" "uuid") RETURNS TABLE("connection_id" "uuid", "status" "text")
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
...
begin
  ...
  insert into public.connections (requester_id, addressee_id, job_id, status)
  values (auth.uid(), v_owner, p_job_id, 'pending')
  on conflict do nothing
  ...
  if v_id is null then
    ...
    where requester_id = auth.uid()
      and addressee_id = v_owner
      and coalesce(job_id,'00000000-0000-0000-0000-000000000000'::uuid)
          = coalesce(p_job_id,'00000000-0000-0000-0000-000000000000'::uuid)
    limit 1;
  end if;
...
ALTER FUNCTION "public"."request_connection"("p_job_id" "uuid") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."set_job_owner"() RETURNS "trigger"
    ...
    AS $$
begin
  if new.posted_by is null and auth.uid() is not null then
    new.posted_by := auth.uid();
  end if;
  return new;
end;
...
CREATE OR REPLACE VIEW "public"."event_attendance_counts" WITH ("security_invoker"='true') AS
 SELECT "e"."id" AS "event_id",
    "count"(*) FILTER (WHERE ("ea"."attendance_status" = ANY (ARRAY['going'::"text", 'checked_in'::"text"]))) AS "total_attendees"
   FROM ("public"."events" "e"
...
CREATE OR REPLACE VIEW "public"."profile_social_links" AS
 SELECT "p"."id",
    "jsonb_build_object"('linkedin', "max"("s"."url") FILTER (WHERE ("s"."type" = 'linkedin'::"public"."social_type")), 'github', "max"("s"."url") FILTER (WHERE ("s"."type" = 'github'::"public"."social_type")), 'x', "max"("s"."url") FILTER (WHERE (("s"."type")::"text" = ANY (ARRAY['x'::"text", 'twitter'::"text"]))), 'website', "max"("s"."url") FILTER (WHERE ("s"."type" = 'website'::"public"."social_type"))) AS "social_links"
   FROM ("public"."profiles" "p"
...
CREATE OR REPLACE VIEW "public"."public_profiles_view" AS
 SELECT "p"."id",
    ...
    "p"."phone"
   FROM "public"."profiles" "p"
  WHERE (("p"."approval_status" = 'approved'::"public"."profile_approval_status") AND (COALESCE("p"."visibility", 'public'::"text") = 'public'::"text") AND (COALESCE("p"."is_deleted", false) = false) AND ("p"."deleted_at" IS NULL));
...
CREATE TABLE IF NOT EXISTS "realtime"."messages_2025_09_10" (
    "topic" "text" NOT NULL,
    "extension" "text" NOT NULL,
    "payload" "jsonb",
    "event" "text",
...
CREATE TABLE IF NOT EXISTS "realtime"."messages_2025_09_11" (
    "topic" "text" NOT NULL,
    "extension" "text" NOT NULL,
    "payload" "jsonb",
    "event" "text",
...
CREATE TABLE IF NOT EXISTS "realtime"."messages_2025_09_12" (
    "topic" "text" NOT NULL,
    "extension" "text" NOT NULL,
    "payload" "jsonb",
    "event" "text",
...
CREATE TABLE IF NOT EXISTS "realtime"."messages_2025_09_13" (
    "topic" "text" NOT NULL,
    "extension" "text" NOT NULL,
    "payload" "jsonb",
    "event" "text",
...
CREATE TABLE IF NOT EXISTS "realtime"."messages_2025_09_14" (
    "topic" "text" NOT NULL,
    "extension" "text" NOT NULL,
    "payload" "jsonb",
    "event" "text",
...
CREATE TABLE IF NOT EXISTS "realtime"."messages_2025_09_15" (
    "topic" "text" NOT NULL,
    "extension" "text" NOT NULL,
    "payload" "jsonb",
    "event" "text",
...
CREATE TABLE IF NOT EXISTS "realtime"."messages_2025_09_16" (
    "topic" "text" NOT NULL,
    "extension" "text" NOT NULL,
    "payload" "jsonb",
    "event" "text",
...
CREATE OR REPLACE TRIGGER "trg_enforce_job_admin_columns" BEFORE UPDATE ON "public"."jobs" FOR EACH ROW EXECUTE FUNCTION "public"."enforce_job_admin_columns"();
...
CREATE OR REPLACE TRIGGER "trg_jobs_admin_cols" BEFORE UPDATE ON "public"."jobs" FOR EACH ROW EXECUTE FUNCTION "public"."enforce_job_admin_columns"();
...
CREATE OR REPLACE TRIGGER "trg_jobs_enforce_admin" BEFORE UPDATE ON "public"."jobs" FOR EACH ROW EXECUTE FUNCTION "public"."enforce_admin_moderation_fields"();
...
CREATE OR REPLACE TRIGGER "trg_set_job_owner" BEFORE INSERT ON "public"."jobs" FOR EACH ROW EXECUTE FUNCTION "public"."set_job_owner"();
...
CREATE POLICY "Public can view active and open-window jobs" ON "public"."jobs" FOR SELECT USING ((("status" = 'active'::"text") AND (("open_at" IS NULL) OR ("now"() >= "open_at")) AND (("close_at" IS NULL) OR ("now"() <= "close_at"))));
...
CREATE POLICY "conn_insert_requester" ON "public"."connections" FOR INSERT TO "authenticated" WITH CHECK (("requester_id" = "auth"."uid"()));
...
CREATE POLICY "conn_update_recipient" ON "public"."connections" FOR UPDATE TO "authenticated" USING (("recipient_id" = "auth"."uid"()));
...
CREATE POLICY "dm_messages_insert" ON "public"."dm_messages" FOR INSERT TO "authenticated" WITH CHECK (( SELECT
        CASE
            WHEN ((("t"."user_a" = "auth"."uid"()) OR ("t"."user_b" = "auth"."uid"())) AND "public"."are_users_connected"("t"."user_a", "t"."user_b")) THEN true
            ELSE false
        END AS "case"
   FROM "public"."dm_threads" "t"
...
CREATE POLICY "dm_messages_select" ON "public"."dm_messages" FOR SELECT TO "authenticated" USING ((( SELECT
        CASE
            WHEN (("t"."user_a" = "auth"."uid"()) OR ("t"."user_b" = "auth"."uid"())) THEN true
            ELSE false
        END AS "case"
   FROM "public"."dm_threads" "t"
  WHERE ("t"."id" = "dm_messages"."thread_id")) AND ("created_at" >= ("now"() - '30 days'::interval))));
...
CREATE POLICY "dm_threads_select" ON "public"."dm_threads" FOR SELECT TO "authenticated" USING (("public"."is_admin"("auth"."uid"()) OR (("auth"."uid"() = "user_a") OR ("auth"."uid"() = "user_b"))));
...
CREATE POLICY "events_delete_owner_admin" ON "public"."events" FOR DELETE TO "authenticated" USING ((("organizer_id" = "auth"."uid"()) OR "public"."is_admin"()));
...
CREATE POLICY "events_insert_employer_admin" ON "public"."events" FOR INSERT TO "authenticated" WITH CHECK (("public"."is_admin"() OR (EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
...
CREATE POLICY "events_update_owner_admin" ON "public"."events" FOR UPDATE TO "authenticated" USING ((("organizer_id" = "auth"."uid"()) OR "public"."is_admin"())) WITH CHECK ((("organizer_id" = "auth"."uid"()) OR "public"."is_admin"()));



CREATE POLICY "gm_delete" ON "public"."group_members" FOR DELETE TO "authenticated" USING ((("user_id" = "auth"."uid"()) OR "public"."is_admin"("auth"."uid"()) OR "public"."is_group_admin"("auth"."uid"(), "group_id")));



CREATE POLICY "gm_insert" ON "public"."group_members" FOR INSERT TO "authenticated" WITH CHECK (((("user_id" = "auth"."uid"()) AND (EXISTS ( SELECT 1
   FROM "public"."groups" "g"
  WHERE (("g"."id" = "group_members"."group_id") AND ("g"."is_private" = false) AND ("g"."is_approved" = true) AND ("g"."is_archived" = false))))) OR "public"."is_admin"("auth"."uid"()) OR "public"."is_group_admin"("auth"."uid"(), "group_id")));



CREATE POLICY "gm_select" ON "public"."group_members" FOR SELECT TO "authenticated" USING ((("user_id" = "auth"."uid"()) OR "public"."is_admin"("auth"."uid"()) OR "public"."is_group_admin"("auth"."uid"(), "group_id")));



CREATE POLICY "gm_update_roles" ON "public"."group_members" FOR UPDATE TO "authenticated" USING (("public"."is_admin"("auth"."uid"()) OR "public"."is_group_admin"("auth"."uid"(), "group_id"))) WITH CHECK (("public"."is_admin"("auth"."uid"()) OR "public"."is_group_admin"("auth"."uid"(), "group_id")));



CREATE POLICY "gp_delete" ON "public"."group_posts" FOR DELETE TO "authenticated" USING ((("user_id" = "auth"."uid"()) OR "public"."is_admin"("auth"."uid"()) OR "public"."is_group_admin"("auth"."uid"(), "group_id")));



CREATE POLICY "gp_insert" ON "public"."group_posts" FOR INSERT TO "authenticated" WITH CHECK ((("user_id" = "auth"."uid"()) AND (EXISTS ( SELECT 1
   FROM "public"."group_members" "gm"
  WHERE (("gm"."group_id" = "group_posts"."group_id") AND ("gm"."user_id" = "auth"."uid"())))) AND (EXISTS ( SELECT 1
   FROM "public"."groups" "g"
  WHERE (("g"."id" = "group_posts"."group_id") AND ("g"."is_archived" = false) AND ((NOT "g"."is_admin_only_posts") OR "public"."is_admin"("auth"."uid"()) OR (EXISTS ( SELECT 1
           FROM "public"."group_members" "gm2"
          WHERE (("gm2"."group_id" = "group_posts"."group_id") AND ("gm2"."user_id" = "auth"."uid"()) AND ("gm2"."role" = 'admin'::"text"))))))))));



CREATE POLICY "gp_select" ON "public"."group_posts" FOR SELECT TO "authenticated" USING (("public"."is_admin"("auth"."uid"()) OR ((EXISTS ( SELECT 1
   FROM "public"."groups" "g"
  WHERE (("g"."id" = "group_posts"."group_id") AND ("g"."is_private" = false) AND ("g"."is_archived" = false)))) AND ("status" = 'approved'::"text")) OR ((EXISTS ( SELECT 1
   FROM "public"."group_members" "gm"
  WHERE (("gm"."group_id" = "group_posts"."group_id") AND ("gm"."user_id" = "auth"."uid"())))) AND (("status" = 'approved'::"text") OR ("user_id" = "auth"."uid"())) AND (EXISTS ( SELECT 1
   FROM "public"."groups" "g"
  WHERE (("g"."id" = "group_posts"."group_id") AND ("g"."is_archived" = false)))))));



CREATE POLICY "gp_update" ON "public"."group_posts" FOR UPDATE TO "authenticated" USING ((("user_id" = "auth"."uid"()) OR "public"."is_admin"("auth"."uid"()) OR "public"."is_group_admin"("auth"."uid"(), "group_id"))) WITH CHECK ((("user_id" = "auth"."uid"()) OR "public"."is_admin"("auth"."uid"()) OR "public"."is_group_admin"("auth"."uid"(), "group_id")));
...
CREATE POLICY "gpr_select_admin" ON "public"."group_post_reports" FOR SELECT TO "authenticated" USING ("public"."is_admin"("auth"."uid"()));
...
CREATE POLICY "gpr_update_admin" ON "public"."group_post_reports" FOR UPDATE TO "authenticated" USING ("public"."is_admin"("auth"."uid"())) WITH CHECK ("public"."is_admin"("auth"."uid"()));
...
CREATE POLICY "group_members_self_join_public" ON "public"."group_members" FOR INSERT TO "authenticated" WITH CHECK ((("user_id" = "auth"."uid"()) AND (EXISTS ( SELECT 1
   FROM "public"."groups" "g"
  WHERE (("g"."id" = "group_members"."group_id") AND ("g"."is_private" = false) AND ("g"."is_approved" = true) AND ("g"."is_archived" = false))))));
...
CREATE POLICY "group_posts_insert_archived" ON "public"."group_posts" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."groups" "g"
  WHERE (("g"."id" = "group_posts"."group_id") AND ("g"."is_archived" = false) AND ((("g"."is_admin_only_posts" = true) AND (EXISTS ( SELECT 1
           FROM "public"."group_members" "gm"
          WHERE (("gm"."group_id" = "g"."id") AND ("gm"."user_id" = "auth"."uid"()) AND ("gm"."role" = 'admin'::"text"))))) OR (("g"."is_admin_only_posts" = false) AND (EXISTS ( SELECT 1
           FROM "public"."group_members" "gm2"
          WHERE (("gm2"."group_id" = "g"."id") AND ("gm2"."user_id" = "auth"."uid"()))))))))));



CREATE POLICY "group_posts_select_archived" ON "public"."group_posts" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."groups" "g"
  WHERE (("g"."id" = "group_posts"."group_id") AND ("public"."_is_admin"("auth"."uid"()) OR (EXISTS ( SELECT 1
           FROM "public"."group_members" "gm"
          WHERE (("gm"."group_id" = "g"."id") AND ("gm"."user_id" = "auth"."uid"()) AND ("gm"."role" = 'admin'::"text")))) OR (("g"."is_archived" = false) AND ((EXISTS ( SELECT 1
           FROM "public"."group_members" "gm2"
          WHERE (("gm2"."group_id" = "g"."id") AND ("gm2"."user_id" = "auth"."uid"())))) OR (("g"."is_private" = false) AND ("g"."is_approved" = true)))))))));
...
CREATE POLICY "groups_delete" ON "public"."groups" FOR DELETE TO "authenticated" USING ((("created_by" = "auth"."uid"()) OR "public"."is_admin"("auth"."uid"()) OR "public"."is_group_admin"("auth"."uid"(), "id")));



CREATE POLICY "groups_insert_any_auth" ON "public"."groups" FOR INSERT TO "authenticated" WITH CHECK (("created_by" = "auth"."uid"()));



CREATE POLICY "groups_select" ON "public"."groups" FOR SELECT TO "authenticated" USING ((("created_by" = "auth"."uid"()) OR "public"."is_admin"("auth"."uid"()) OR "public"."is_group_admin"("auth"."uid"(), "id") OR (("is_approved" = true) AND ("is_archived" = false))));
...
CREATE POLICY "groups_update" ON "public"."groups" FOR UPDATE TO "authenticated" USING ((("created_by" = "auth"."uid"()) OR "public"."is_admin"("auth"."uid"()) OR "public"."is_group_admin"("auth"."uid"(), "id"))) WITH CHECK ((("created_by" = "auth"."uid"()) OR "public"."is_admin"("auth"."uid"()) OR "public"."is_group_admin"("auth"."uid"(), "id")));
...
CREATE POLICY "ja_delete_scope" ON "public"."job_applications" FOR DELETE TO "authenticated" USING ((("applicant_id" = "auth"."uid"()) OR "public"."is_admin"()));



CREATE POLICY "ja_insert_self" ON "public"."job_applications" FOR INSERT TO "authenticated" WITH CHECK (("applicant_id" = "auth"."uid"()));



CREATE POLICY "ja_read_scope" ON "public"."job_applications" FOR SELECT TO "authenticated" USING ((("applicant_id" = "auth"."uid"()) OR "public"."is_admin"() OR (EXISTS ( SELECT 1
   FROM "public"."jobs" "j"
  WHERE (("j"."id" = "job_applications"."job_id") AND ("j"."posted_by" = "auth"."uid"()))))));



CREATE POLICY "ja_update_scope" ON "public"."job_applications" FOR UPDATE TO "authenticated" USING ((("applicant_id" = "auth"."uid"()) OR "public"."is_admin"() OR (EXISTS ( SELECT 1
   FROM "public"."jobs" "j"
  WHERE (("j"."id" = "job_applications"."job_id") AND ("j"."posted_by" = "auth"."uid"())))))) WITH CHECK (true);
...
CREATE POLICY "job_app read applicant or poster or admin" ON "public"."job_applications" FOR SELECT TO "authenticated" USING ((("applicant_id" = "auth"."uid"()) OR (EXISTS ( SELECT 1
   FROM "public"."jobs" "j"
  WHERE (("j"."id" = "job_applications"."job_id") AND ("j"."posted_by" = "auth"."uid"())))) OR "public"."is_site_admin"()));
...
CREATE POLICY "job_app update status by poster or admin" ON "public"."job_applications" FOR UPDATE TO "authenticated" USING ((("applicant_id" = "auth"."uid"()) OR (EXISTS ( SELECT 1
   FROM "public"."jobs" "j"
  WHERE (("j"."id" = "job_applications"."job_id") AND ("j"."posted_by" = "auth"."uid"())))) OR "public"."is_site_admin"())) WITH CHECK (true);
...
CREATE POLICY "job_applications_insert" ON "public"."job_applications" FOR INSERT TO "authenticated" WITH CHECK (("applicant_id" = "auth"."uid"()));



CREATE POLICY "job_apps_insert" ON "public"."job_applications" FOR INSERT TO "authenticated" WITH CHECK (("applicant_id" = "auth"."uid"()));



CREATE POLICY "job_apps_select" ON "public"."job_applications" FOR SELECT TO "authenticated" USING ((("applicant_id" = "auth"."uid"()) OR (EXISTS ( SELECT 1
   FROM "public"."jobs" "j"
  WHERE (("j"."id" = "job_applications"."job_id") AND (("j"."posted_by" = "auth"."uid"()) OR ("j"."user_id" = "auth"."uid"()) OR (EXISTS ( SELECT 1
           FROM "public"."profiles" "p"
          WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."app_role_enum", 'super_admin'::"public"."app_role_enum"])))))))))));



CREATE POLICY "job_apps_update" ON "public"."job_applications" FOR UPDATE TO "authenticated" USING (((EXISTS ( SELECT 1
   FROM "public"."jobs" "j"
  WHERE (("j"."id" = "job_applications"."job_id") AND (("j"."posted_by" = "auth"."uid"()) OR ("j"."user_id" = "auth"."uid"()))))) OR (EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."app_role_enum", 'super_admin'::"public"."app_role_enum"])))))));
...
CREATE POLICY "jobs_delete" ON "public"."jobs" FOR DELETE TO "authenticated" USING ((("posted_by" = "auth"."uid"()) OR "public"."is_site_admin"()));



CREATE POLICY "jobs_delete_owner" ON "public"."jobs" FOR DELETE TO "authenticated" USING ((("posted_by" = "auth"."uid"()) OR ("user_id" = "auth"."uid"()) OR (EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."app_role_enum", 'super_admin'::"public"."app_role_enum"])))))));



CREATE POLICY "jobs_delete_owner_admin" ON "public"."jobs" FOR DELETE TO "authenticated" USING ((("posted_by" = "auth"."uid"()) OR "public"."is_admin"()));



CREATE POLICY "jobs_delete_owner_or_admin" ON "public"."jobs" FOR DELETE TO "authenticated" USING ((("user_id" = "auth"."uid"()) OR "public"."is_site_admin"()));



CREATE POLICY "jobs_employer_insert" ON "public"."jobs" FOR INSERT TO "authenticated" WITH CHECK ((("auth"."uid"() IS NOT NULL) AND (EXISTS ( SELECT 1
   FROM "public"."profiles" "pr"
  WHERE (("pr"."id" = "auth"."uid"()) AND (("pr"."role" = 'employer'::"public"."app_role_enum") OR (COALESCE("pr"."is_employer", false) = true)))))));



CREATE POLICY "jobs_insert" ON "public"."jobs" FOR INSERT TO "authenticated" WITH CHECK (("posted_by" = "auth"."uid"()));



CREATE POLICY "jobs_insert_employer_or_admin" ON "public"."jobs" FOR INSERT TO "authenticated" WITH CHECK (("public"."current_role"() = ANY (ARRAY['employer'::"public"."app_role_enum", 'admin'::"public"."app_role_enum", 'super_admin'::"public"."app_role_enum"])));



CREATE POLICY "jobs_insert_employers" ON "public"."jobs" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND (("p"."role" = 'employer'::"public"."app_role_enum") OR COALESCE("p"."is_employer", false))))));



CREATE POLICY "jobs_read_public" ON "public"."jobs" FOR SELECT TO "authenticated", "anon" USING ((((COALESCE("is_approved", false) = true) AND (COALESCE("is_active", true) = true)) OR ("posted_by" = "auth"."uid"()) OR "public"."is_admin"()));



CREATE POLICY "jobs_select" ON "public"."jobs" FOR SELECT TO "authenticated", "anon" USING (((("is_approved" = true) AND ("is_active" = true)) OR ("posted_by" = "auth"."uid"()) OR "public"."is_site_admin"()));



CREATE POLICY "jobs_select_active_approved" ON "public"."jobs" FOR SELECT USING ((("is_active" = true) AND ("is_approved" = true)));



CREATE POLICY "jobs_select_owner_or_company" ON "public"."jobs" FOR SELECT TO "authenticated" USING ((("posted_by" = "auth"."uid"()) OR ("user_id" = "auth"."uid"()) OR (EXISTS ( SELECT 1
   FROM "public"."companies" "c"
  WHERE (("c"."id" = "jobs"."company_id") AND ("c"."created_by" = "auth"."uid"())))) OR (EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."app_role_enum", 'super_admin'::"public"."app_role_enum"])))))));



CREATE POLICY "jobs_update" ON "public"."jobs" FOR UPDATE TO "authenticated" USING ((("posted_by" = "auth"."uid"()) OR "public"."is_site_admin"())) WITH CHECK ((("posted_by" = "auth"."uid"()) OR "public"."is_site_admin"()));



CREATE POLICY "jobs_update_owner" ON "public"."jobs" FOR UPDATE TO "authenticated" USING ((("posted_by" = "auth"."uid"()) OR ("user_id" = "auth"."uid"()) OR (EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."app_role_enum", 'super_admin'::"public"."app_role_enum"]))))))) WITH CHECK ((("posted_by" = "auth"."uid"()) OR ("user_id" = "auth"."uid"()) OR (EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."app_role_enum", 'super_admin'::"public"."app_role_enum"])))))));



CREATE POLICY "jobs_update_owner_admin" ON "public"."jobs" FOR UPDATE TO "authenticated" USING ((("posted_by" = "auth"."uid"()) OR "public"."is_admin"())) WITH CHECK ((("posted_by" = "auth"."uid"()) OR "public"."is_admin"()));
...
CREATE POLICY "mentors_insert" ON "public"."mentors" FOR INSERT TO "authenticated" WITH CHECK ((("user_id" = "auth"."uid"()) OR "public"."is_admin"("auth"."uid"())));



CREATE POLICY "mentors_select" ON "public"."mentors" FOR SELECT TO "authenticated" USING ((("user_id" = "auth"."uid"()) OR ("status" = 'approved'::"text") OR "public"."is_admin"("auth"."uid"())));



CREATE POLICY "mentors_update" ON "public"."mentors" FOR UPDATE TO "authenticated" USING ((("user_id" = "auth"."uid"()) OR "public"."is_admin"("auth"."uid"()))) WITH CHECK ((("user_id" = "auth"."uid"()) OR "public"."is_admin"("auth"."uid"())));
...
CREATE POLICY "mentorship_messages_delete" ON "public"."mentorship_messages" FOR DELETE TO "authenticated" USING (("sender_id" = "auth"."uid"()));



CREATE POLICY "mentorship_messages_insert" ON "public"."mentorship_messages" FOR INSERT TO "authenticated" WITH CHECK ((("sender_id" = "auth"."uid"()) AND (EXISTS ( SELECT 1
   FROM "public"."mentorship_requests" "r"
  WHERE (("r"."id" = "mentorship_messages"."mentorship_request_id") AND (("r"."mentee_id" = "auth"."uid"()) OR ("r"."mentor_id" = "auth"."uid"())))))));



CREATE POLICY "mentorship_messages_select" ON "public"."mentorship_messages" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."mentorship_requests" "r"
  WHERE (("r"."id" = "mentorship_messages"."mentorship_request_id") AND (("r"."mentee_id" = "auth"."uid"()) OR ("r"."mentor_id" = "auth"."uid"()))))));



CREATE POLICY "mentorship_messages_update" ON "public"."mentorship_messages" FOR UPDATE TO "authenticated" USING (("sender_id" = "auth"."uid"())) WITH CHECK (("sender_id" = "auth"."uid"()));
...
CREATE POLICY "mentorship_requests_insert" ON "public"."mentorship_requests" FOR INSERT TO "authenticated" WITH CHECK (((("mentee_id" = "auth"."uid"()) OR "public"."is_admin"("auth"."uid"()) OR "public"."is_super_admin"("auth"."uid"())) AND (EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "mentorship_requests"."mentor_id") AND (COALESCE("p"."is_available_for_mentorship", false) = true))))));



CREATE POLICY "mentorship_requests_select" ON "public"."mentorship_requests" FOR SELECT TO "authenticated" USING ((("mentee_id" = "auth"."uid"()) OR ("mentor_id" = "auth"."uid"()) OR "public"."is_admin"("auth"."uid"())));



CREATE POLICY "mentorship_requests_update" ON "public"."mentorship_requests" FOR UPDATE TO "authenticated" USING ((("mentee_id" = "auth"."uid"()) OR ("mentor_id" = "auth"."uid"()) OR "public"."is_admin"("auth"."uid"()))) WITH CHECK ((("mentee_id" = "auth"."uid"()) OR ("mentor_id" = "auth"."uid"()) OR "public"."is_admin"("auth"."uid"())));
...
CREATE POLICY "mentorship_sessions_insert" ON "public"."mentorship_sessions" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."mentorship_requests" "r"
  WHERE (("r"."id" = "mentorship_sessions"."mentorship_request_id") AND ("r"."status" = 'accepted'::"text") AND (("r"."mentor_id" = "auth"."uid"()) OR "public"."is_admin"("auth"."uid"()))))));



CREATE POLICY "mentorship_sessions_insert_policy" ON "public"."mentorship_sessions" FOR INSERT WITH CHECK (((EXISTS ( SELECT 1
   FROM "public"."mentorship_requests" "mr"
  WHERE (("mr"."id" = "mentorship_sessions"."mentorship_request_id") AND ("mr"."mentor_id" = "auth"."uid"()) AND ("mr"."status" = 'accepted'::"text")))) OR ("public"."get_user_role"("auth"."uid"()) = ANY (ARRAY['admin'::"text", 'super_admin'::"text"]))));



CREATE POLICY "mentorship_sessions_select" ON "public"."mentorship_sessions" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."mentorship_requests" "r"
  WHERE (("r"."id" = "mentorship_sessions"."mentorship_request_id") AND (("r"."mentee_id" = "auth"."uid"()) OR ("r"."mentor_id" = "auth"."uid"()) OR "public"."is_admin"("auth"."uid"()))))));
...
CREATE POLICY "mentorship_sessions_update" ON "public"."mentorship_sessions" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."mentorship_requests" "r"
  WHERE (("r"."id" = "mentorship_sessions"."mentorship_request_id") AND (("r"."mentor_id" = "auth"."uid"()) OR "public"."is_admin"("auth"."uid"())))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."mentorship_requests" "r"
  WHERE (("r"."id" = "mentorship_sessions"."mentorship_request_id") AND (("r"."mentor_id" = "auth"."uid"()) OR "public"."is_admin"("auth"."uid"()))))));
...
CREATE POLICY "profiles_select_admin" ON "public"."profiles" FOR SELECT TO "authenticated" USING ("public"."is_site_admin"());
...
CREATE POLICY "profiles_select_directory" ON "public"."profiles" FOR SELECT TO "authenticated" USING ((("is_active" = true) AND (COALESCE("is_approved", false) = true)));
...
CREATE POLICY "profiles_update_admin" ON "public"."profiles" FOR UPDATE TO "authenticated" USING ("public"."is_site_admin"()) WITH CHECK ("public"."is_site_admin"());
...
CREATE POLICY "users can insert their own dm_threads" ON "public"."dm_threads" FOR INSERT WITH CHECK ((("user_a" = "auth"."uid"()) OR ("user_b" = "auth"."uid"())));
...
CREATE POLICY "resumes delete own or admin" ON "storage"."objects" FOR DELETE TO "authenticated" USING ((("bucket_id" = 'resumes'::"text") AND (("split_part"("name", '/'::"text", 1) = ("auth"."uid"())::"text") OR "public"."is_site_admin"())));
...
CREATE POLICY "resumes read by job poster or admin" ON "storage"."objects" FOR SELECT TO "authenticated" USING ((("bucket_id" = 'resumes'::"text") AND (EXISTS ( SELECT 1
  ...
  WHERE (("ja"."resume_url" = "objects"."name") AND (("j"."posted_by" = "auth"."uid"()) OR "public"."is_site_admin"()))))));
...
CREATE POLICY "resumes_delete_own" ON "storage"."objects" FOR DELETE TO "authenticated" USING ((("bucket_id" = 'resumes'::"text") AND ("name" ~~ (("auth"."uid"())::"text" || '/%'::"text"))));



CREATE POLICY "resumes_insert_own" ON "storage"."objects" FOR INSERT TO "authenticated" WITH CHECK ((("bucket_id" = 'resumes'::"text") AND ("name" ~~ (("auth"."uid"())::"text" || '/%'::"text"))));



CREATE POLICY "resumes_read_own" ON "storage"."objects" FOR SELECT TO "authenticated" USING ((("bucket_id" = 'resumes'::"text") AND ("name" ~~ (("auth"."uid"())::"text" || '/%'::"text"))));



CREATE POLICY "resumes_update_own" ON "storage"."objects" FOR UPDATE TO "authenticated" USING ((("bucket_id" = 'resumes'::"text") AND ("name" ~~ (("auth"."uid"())::"text" || '/%'::"text")))) WITH CHECK ((("bucket_id" = 'resumes'::"text") AND ("name" ~~ (("auth"."uid"())::"text" || '/%'::"text"))));
...
GRANT ALL ON FUNCTION "public"."admin_set_roles"("p_user_ids" "uuid"[], "p_role" "public"."app_role_enum") TO "anon";
...
GRANT ALL ON FUNCTION "public"."admin_set_user_role"("p_user_id" "uuid", "p_role" "public"."app_role_enum") TO "anon";
GRANT ALL ON FUNCTION "public"."admin_set_user_role"("p_user_id" "uuid", "p_role" "public"."app_role_enum") TO "authenticated";
...
GRANT ALL ON FUNCTION "public"."get_jobs_with_bookmarks_v3"("p_search" "text", "p_limit" integer, "p_offset" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_jobs_with_bookmarks_v3"("p_search" "text", "p_limit" integer, "p_offset" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_jobs_with_bookmarks_v3"("p_search" "text", "p_limit" integer, "p_offset" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_jobs_with_bookmarks_v3"("p_search_query" "text", "p_sort_by" "text", "p_sort_order" "text", "p_limit" integer, "p_offset" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_jobs_with_bookmarks_v3"("p_search_query" "text", "p_sort_by" "text", "p_sort_order" "text", "p_limit" integer, "p_offset" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_jobs_with_bookmarks_v3"("p_search_query" "text", "p_sort_by" "text", "p_sort_order" "text", "p_limit" integer, "p_offset" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_jobs_with_bookmarks_v3"("p_search_query" "text", "p_sort_by" "text", "p_sort_order" "text", "p_limit" integer, "p_offset" integer, "p_department" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."get_jobs_with_bookmarks_v3"("p_search_query" "text", "p_sort_by" "text", "p_sort_order" "text", "p_limit" integer, "p_offset" integer, "p_department" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_jobs_with_bookmarks_v3"("p_search_query" "text", "p_sort_by" "text", "p_sort_order" "text", "p_limit" integer, "p_offset" integer, "p_department" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_my_posted_jobs"("p_limit" integer, "p_offset" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_my_posted_jobs"("p_limit" integer, "p_offset" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_my_posted_jobs"("p_limit" integer, "p_offset" integer) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_recent_activity"() TO "anon";
GRANT ALL ON FUNCTION "public"."get_recent_activity"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_recent_activity"() TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."is_group_admin"("p_user_id" "uuid", "p_group_id" "uuid") FROM PUBLIC;
...
GRANT ALL ON FUNCTION "public"."request_connection"("p_job_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."request_connection"("p_job_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."request_connection"("p_job_id" "uuid") TO "service_role";
...
ALTER ROLE "cli_login_postgres" WITH NOINHERIT NOCREATEROLE NOCREATEDB LOGIN NOBYPASSRLS VALID UNTIL '2025-09-12 06:11:04.528999+00';
