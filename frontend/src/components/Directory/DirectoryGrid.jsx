...
export default function DirectoryGrid({ items = [], meId, currentTab = 'all', onChanged, compact = false, loading = false }) {
  const Skeleton = () => (
    <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm animate-pulse" aria-hidden="true">
  ...
  if (loading) {
    ...
    return (
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3" role="status" aria-label="Loading alumni profiles">
  ...
  if (!Array.isArray(items) || items.length === 0) {
    return (
      <div className="rounded-xl border border-slate-200 bg-white p-10 text-center text-slate-500 shadow-sm">
        <p className="font-medium">No alumni match your search.</p>
