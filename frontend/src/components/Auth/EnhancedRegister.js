...
import { EyeIcon, EyeSlashIcon, CheckIcon } from '@heroicons/react/24/outline'; // Removed XMarkIcon as it's not used in the final version
...
const EnhancedRegister = () => {
    ...
    const fetchRoles = async () => {
      ...
      try {
        const { data, error: fetchError } = await supabase
          ...
          .not('name', 'in', '("super_admin", "admin", "moderator")'); // Exclude admin-level roles
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validateStep(1) || !validateStep(2) || !validateStep(3)) {
      // Ensure all steps are validated before final submission, though current flow validates step-by-step
      setError("Please ensure all required fields are correctly filled in all steps.");
      return;
    }
    ...
    const userMetadata = {
      ...
      experience_years: formData.interestedInMentorship && (formData.mentorshipRole === 'mentor' || formData.mentorshipRole === 'both') ? parseInt(formData.experienceYears, 10) : null,
      skills: formData.interestedInMentorship ? formData.skills : [],
      interests: formData.interestedInMentorship ? formData.interests : [],
      mentorship_goals: formData.interestedInMentorship ? formData.mentorshipGoals.trim() : '',
    ...
    try {
      const { data, error: signUpError } = await signUpWithEmail(formData.email, formData.password, userMetadata);
      ...
      if (formData.primaryRole === 'employer') {
        successMessage = "Registration submitted! Your account is pending admin approval. You'll be notified by email upon activation.";
      } else if (formData.interestedInMentorship) {
        successMessage = "Registration successful! Your mentorship application is under review. Please check your email to verify your account.";
      }
      setError(successMessage); // Using 'error' state to display success message as well
      // Consider navigating after a delay or on user action
      ...
      <div className="max-w-2xl w-full space-y-8">
        <div className="text-center">
          <Link to="/" className="inline-block mb-6">
            ...
            <div className="mx-auto w-20 h-20 bg-blue-600 rounded-full flex items-center justify-center shadow-xl hover:bg-blue-700 transition-colors">
